...
LOCAL_MODULE     := CrystalBall
LOCAL_C_INCLUDES := $(LOCAL_PATH) $(LOCAL_PATH)/Sources
LOCAL_SRC_FILES  := JNI.cpp                                               \
                       $(call LS_CPP,$(LOCAL_PATH),Sources)                \
                       $(call LS_CPP,$(LOCAL_PATH),Sources/Level)          \
                       $(call LS_CPP,$(LOCAL_PATH),Sources/Level/Ball)     \
                       $(call LS_CPP,$(LOCAL_PATH),Sources/Level/Title)    \
                       $(call LS_CPP,$(LOCAL_PATH),Sources/Level/Sphere)   \
                       $(call LS_CPP,$(LOCAL_PATH),Sources/Level/Response)
LOCAL_CPPFLAGS         := -fPIC -fexceptions -Wmultichar -ffunction-sections -fdata-sections -std=c++98 -std=gnu++98 -fno-rtti
